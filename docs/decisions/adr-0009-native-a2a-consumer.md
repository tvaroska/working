...
- **Status:** Accepted
- **Date:** 2026-08-15
- **Scope:** the **native-consumer principle + wire vocabulary** (`RemoteA2aAgent`; `input-required` pause; `TaskStatusUpdateEvent.status.message` progress; `use_legacy` pin). *How* the consumer hangs off the caller (transfer / `AgentTool` / `Workflow` graph) and *how* state is restored are decided in **`adr-0010-durable-consumer-construct.md`**.
- **Supersedes (consumer side):** the hand-rolled `BridgeClient` port + `A2ABridgeClient` + `FunctionTool` consumer introduced for the M0 tracer bullet (`agents/src/bridge_client/`, M0.3/M0.5). The port was subsequently **removed** (not retained as a double); the wiring trajectory that followed is recorded in `adr-0010`.
...
- **The `BridgeClient` port + `A2ABridgeClient` become tracer-bullet-only** — and were **subsequently removed** (the address agent consumes the Bridge natively; see `adr-0010` for the wiring trajectory). Sprint 1 grows against `RemoteA2aAgent`, not behind the port.
...
## Wiring and state restore → `adr-0010`

The consumer *construct/wiring* trajectory — M0 port removed → `transfer` sub-agent → `AgentTool` interim (S1-2/S1-4, with the exchange `context_id` threaded through session state) → **native `Workflow` graph** as the durable target — and the **durable state-restore** breakdown (DEFAULT persistent stores + EXPERIMENTAL `ResumabilityConfig` + a small CUSTOM webhook receiver/index; "the webhook is a doorbell, not a restore") are decided in **`docs/decisions/adr-0010-durable-consumer-construct.md`**. Lessons: `docs/lessons-learned.md` A12–A13. Build task: `PLAN.md` S1-6.
