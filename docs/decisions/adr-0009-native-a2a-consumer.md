...
- **Open consequence — structured `CollectRequest` on the send path.** Under transfer, the consumer forwards conversation content, not a JSON `CollectRequest` DataPart, so the structured outbound request no longer travels on the agent's send. The mock Bridge is content-agnostic, so the M0 round-trip still passes; carrying `CollectRequest` (and post-processing the returned ledger for the `is_satisfied` gate — which needs control to return to the address agent, e.g. via `AgentTool` or a follow-up root turn) is **Sprint-1 work**.
  - **Closed (2026-08-15, S1-2):** the address agent now consumes the Bridge as a **`BridgeAgentTool`** (an `AgentTool` over the same `RemoteA2aAgent`) — call-and-return, so control returns to the caller with the `ExchangeTurn` as the tool result. The structured `CollectRequest` DataPart is restored on the send path by a `RequestInterceptor` (`build_collect_request_interceptor`) baked into the consumer, guarded on `task_id` so it rewrites only the fresh send (not the S1-4 resume). Card-URL swap unchanged.
- **Unchanged:** the wire vocabulary (§2–§4), `use_legacy=True` pin (§5), and the experimental-surface risk all still hold.

### Amendment (2026-08-15) — S1-4 multi-turn Collect loop: exchange **context** threaded across rounds

The Collect loop landed (address agent: `document_bridge` collect → authoritative `check_completeness` gate → chase if not done → terminate on satisfied). Two decisions worth recording, because they interact with §2's park/resume model and could otherwise read as regressions:

- **Durable unit across the loop is the exchange *context*, not a reused *task*.** Each `AgentTool` call runs the wrapped `RemoteA2aAgent` in a **fresh child session** (new `InMemorySessionService`, no prior events), so the native context/task ids the agent persists on its own event history (`_construct_message_parts_from_session`) are wiped between rounds. Parent→child **session state** is the only channel that survives an `AgentTool` invocation. S1-4 therefore threads the A2A `context_id` through state: `BridgeAgentTool` writes the returned turn's `context_id` under `bridge_exchange_context_id` (bridge-owned key, `EXCHANGE_CONTEXT_STATE_KEY`), and the send-path interceptor stamps it on the next round's `message/send`. Every round continues the **same exchange** — asserted live (`MockBridgeExecutor.context_ids_seen` identical across rounds).
- **The A2A `task_id` is deliberately *not* reused across rounds in the `park=False` completing path.** Each round runs the mock's hold→`COMPLETED` path, so each send legitimately opens a **new task under the same context** (A2A: a context groups tasks; re-sending to a *completed* task is not valid). "One task reused across turns" applies only to the **parked (`input-required`) native pause/resume** timescale (§2), whose cross-`AgentTool` durability (fresh-child-session limitation) is a **separate, later concern** (S1-5 / Phase 3), not solved here.
- **Implementation note (loop mechanics):** the loop's Bridge tool is built with `skip_summarization=False`. An ADK function-response event with `skip_summarization` set reports `is_final_response()==True`, which would end the turn immediately after the collect and starve the gate; keeping summarization on re-invokes the model after each collect so it can route to `check_completeness`. The gate stays authoritative — the model routes on its verdict, it never mints "done" (docs/lessons-learned.md A3).
