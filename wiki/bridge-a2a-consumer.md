...
`RemoteA2aAgent` is *what* consumes the Bridge; the **target shape of the whole agent** is the same for every service agent — Address, Benefits, RFP, and any internal agent that calls the Bridge (`adr-0010`). That reusable whole-agent shape has its own reference atom — **[[bridge-service-agent-architecture|service-agent reference architecture]]** — the pattern to instantiate for a new use case. In brief:

> a **`Workflow`** (native graph) whose nodes are **(1) a `RemoteA2aAgent` collect node** (calls the Bridge over canonical A2A), **(2) a deterministic gate node** (the agent's Sense-B "done" function — `is_satisfied` for Address; the mutating-requirements decision for RFP), branching back to collect or forward, and **(3) a presenter node** (renders the deliverable — a code pass-through today, or an `LlmAgent` for a natural-language summary) — all on **one shared, durable session**.

The reusability thesis cuts one level deeper: the difference between demos is the **nodes and the gate**, not the graph *kind*. Two invariants make this a *shape*, not a suggestion: the gate node is a **deterministic pure function** (a model may never mint "done" — Sense B, [[bridge-collect]]), and the graph sits **on top** of the LLM (any LLM appears as a *node*, e.g. the presenter) because "`Workflow` cannot yet be used as an `LlmAgent` sub-agent" (`google-adk` 2.7.0). Full node-by-node spec, the fixed/varies table, and the instantiation gotchas live in [[bridge-service-agent-architecture]].
...
| **`AgentTool`** | **Yes**, as a tool result | **fresh throwaway** per call | **impossible** | interim (M0/S1) — **retired**, git history only |
...
**Current state.** S1-6 landed the durable graph (`agents/address/graph.py`) as a `Workflow` conditional cycle on one shared, durable session (`DatabaseSessionService` + `DatabaseTaskStore` + `ResumabilityConfig`), proven to park at `input-required` and resume to the same state across a process restart with no HTTP. The durable graph is now **the** address consumer: the interim `AgentTool` wiring (`agent.py`'s `BridgeAgentTool` call-and-return and the `LlmAgent` two-tool Collect loop) has been **retired** — deleted from the tree, kept in git history — now that the graph supersedes it (`adr-0010`). What survives from that path is the still-shared machinery it first proved out: the send-path `task_id`-guarded `RequestInterceptor` (`bridge_client/remote_consumer.py`), the durable exchange `context_id` state key (`EXCHANGE_CONTEXT_STATE_KEY`), and the pure A2A wire helpers — all now consumed by the graph's collect node. Build history: `PLAN.md` (M0.x superseded; S1-2/S1-4/S1-6 landed, interim wiring retired); rationale: `adr-0010`.
...
**Consequence for sequencing:** durable park/resume across a restart needs only the DEFAULT stores + the EXPERIMENTAL switch — **no HTTP, testable by resuming via `run_async` manually.** That work (S1-6) de-risks the webhook path, which adds only the two CUSTOM pieces and only when a leg must outlive the process. How this maps onto the deployed substrate (Cloud Run runtime, Vertex/Agent Engine sessions, where the wake receiver lives) is [[bridge-deploy-resume|its own atom]].
...
- **Landed (S1-2/S1-3/S1-4/S1-5), interim wiring since retired:** native consumer as a `BridgeAgentTool` call-and-return; `CollectRequest` restored via a `task_id`-guarded interceptor; the authoritative `check_completeness` gate; the multi-turn Collect loop threading one durable exchange `context_id` across rounds; the mock grown into the multi-turn contract double. The `BridgeAgentTool`/`LlmAgent`-loop pieces were **retired** when the S1-6 graph superseded them (git history only); the interceptor, `context_id` threading, the deterministic gate (`is_satisfied`, called directly by the graph — the S1-3 `check_completeness` `ToolContext` wrapper was dropped with the `AgentTool` loop), and the mock carried forward. Details in `PLAN.md` + `adr-0010`.
- **Landed (S1-6):** the durable graph consumer (`agents/address/graph.py`) — a `Workflow` conditional cycle (`RemoteA2aAgent` collect node → deterministic `is_satisfied` gate → present, with a routed `gate --[again]--> collect` loop-back) on one shared, durable session; proven to park at `input-required` and resume to the same state across a process restart with no HTTP, via `DatabaseSessionService` + `DatabaseTaskStore` + `ResumabilityConfig`. A send-path interceptor re-detects the resume under the graph's event ordering (`RemoteA2aAgent`'s own last-event heuristic misses it). Supersedes and replaces the interim `AgentTool` wiring, now retired to git history. Details in `PLAN.md` + `adr-0012`.
