...
| **native graph** (`LoopAgent`, the `Workflow` fallback) | **Yes** — the loop *is* the control flow; the gate is a sub-agent | **shared, durable** — sub-agents get the parent `InvocationContext`; `RemoteA2aAgent` sees its own prior event and reuses the `context_id` natively | **works** — an `input-required` pause hits `ctx.should_pause_invocation`, the loop suspends and resumes from saved state | **landed (S1-6)** — `agents/address/graph.py` |
...
- **`Workflow` (node/edge graph, `google.adk.workflow`) is the go-forward API and is *not* `@experimental`.** `LoopAgent`/`SequentialAgent` are now `@deprecated` **in favor of `Workflow`** (`loop_agent.py`), so the durable design *targeted* `Workflow` — but S1-6's spike found `Workflow` **cannot re-enter a completed node** (the scheduler fast-forwards any node that already COMPLETED in the current invocation, `check_interception` Case 1), so a conditional not-done→collect loop-back never iterates. The build therefore **fell back to `LoopAgent`** (its pre-recorded fallback: re-runs sub-agents each iteration on the shared session, propagates the pause, terminates on `escalate`). Deprecation risk is now a live SDK-risk item (`adr-0012`); migrate to `Workflow` when a release permits node re-entry / a native loop node.
- **The two S1-6 spike gates, resolved:** (1) a `RemoteA2aAgent` (a non-`LlmAgent` `BaseAgent`) runs correctly as a graph sub-agent — **confirmed** (load-bearing detail: its `ExchangeTurn` artifact must be emitted `last_chunk=True` or the *partial* event streams but is never persisted to the shared session, so the gate can't read it — [[bridge-adk|lessons A12]]); (2) an `input-required` pause propagates/resumes cleanly **and survives a process restart** — confirmed inside `LoopAgent` (the `Workflow` loop-back re-entry is what failed, not the pause mechanism). The composition constraint still holds: **"`Workflow` cannot yet be used as an `LlmAgent` sub-agent"** — so a graph sits **on top** (LLM as a *node*, e.g. a presenter), not behind an LLM "front".
...
- **Landed (S1-6):** the durable graph consumer (`agents/address/graph.py`) — a `LoopAgent` (`RemoteA2aAgent` collect sub-agent → deterministic `is_satisfied` gate) on one shared, durable session; proven to park at `input-required` and resume to the same state across a process restart with no HTTP, via `DatabaseSessionService` + `DatabaseTaskStore` + `ResumabilityConfig`. Lands alongside the `AgentTool` wiring (both kept). Details in `PLAN.md` + `adr-0012`.
