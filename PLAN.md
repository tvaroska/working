...
- [ ] **S1-2 — Control-return wiring (transfer → `AgentTool`).** Switch the Bridge consumer from a transfer `sub_agent` to `AgentTool` (call-and-return) so control returns to the address agent with the `ExchangeTurn`, and restore the structured `CollectRequest` as a JSON DataPart on the send path (transfer currently forwards conversation content — adr-0009 amendment open consequence). Card-URL swap unchanged; cover in the seam suite. *(Depends on S1-1.)*
