...
- [ ] **M0.1 — Scaffold.** `uv` project, Python 3.12+, `agents/` package; pin `google-adk >= 2.7.0,<3` and `a2a-sdk`; add pytest + ruff. (Minimal slice of Sprint-0 scaffolding — only what M0 needs.)
...
- [ ] **M0.6 — Round-trip test.** One pytest: agent→mock→agent; assert payload matches `expected.json` entry, and assert `WORKING` was observed before `COMPLETED` (async path exercised). 10s hold shrinkable for the test.
